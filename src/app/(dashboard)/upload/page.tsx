...
export default function UploadPage() {
            ...
            if (newProgress >= 100) {
              ...
              setTimeout(() => {
                ...
                const processInterval = setInterval(() => {
                  setFiles(prev3 => prev3.map(f3 => {
                    if (f3.id === newFile.id && f3.status === 'processing') {
                      const newProgress = Math.min(f3.progress + Math.random() * 15, 100);
                      if (newProgress >= 100) {
  ...
  const handleDownloadFile = (file: UploadedFile) => {
    ...
    setFiles(prev => prev.map(f => 
      f.id === file.id 
        ? { ...f, downloadCount: f.downloadCount + 1, lastAccessed: new Date() }
        : f
    ));
  };
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{stats.uploadsThisMonth}</p>
                <div className="mt-2">
                  <Progress 
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <h3 className="font-semibold text-lg truncate">{file.originalName}</h3>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600 mb-3">
                          ...
                          <div>
                            <span className="font-medium">Type:</span> {getFileExtension(file.name).toUpperCase()}
                          </div>
                          <div>
                            <span className="font-medium">Uploaded:</span> {formatDistanceToNow(file.uploadedAt, { addSuffix: true, locale: id })}
                          </div>
                          <div>
                        ...
                        {file.metadata.rows && (
                          <div className="flex gap-4 text-sm text-gray-600 mb-3">
                            <span><strong>Rows:</strong> {file.metadata.rows.toLocaleString()}</span>
                            <span><strong>Columns:</strong> {file.metadata.columns}</span>
                            {file.metadata.sheets && (
                              <span><strong>Sheets:</strong> {file.metadata.sheets.length}</span>
                            )}
                          </div>
                        )}
